...
class Solver:
    def __init__(self, url, sitekey, apikey, headless = False, userDataPath = None):
        ...
        self.nocaptchaai = {
            "apikey": apikey,
            "solver": "https://pro.nocaptchaai.com/solve"
        }
    ...
    async def _getHsw(self, m, c):
        try:
            try:
                browser = await pyppeteer.launch({"autoClose": False, "headless": True, 'executablePath': chrome, "userDataDir": self.userDataPath, "args": []})
            except:
    ...
    async def _getCaptcha(self):
        ...
        # print(self.s, o["c"]["req"])
        hsw = await self._getHsw(self.s, o["c"]["req"])
        # print(hsw)
...
async def manage():
    ...
    if result == False:
        ...
        return False
    elif result == "Something wrong":
        return "Something wrong"
    else:
        print("Captcha Key is")
        print(result)
        return result
